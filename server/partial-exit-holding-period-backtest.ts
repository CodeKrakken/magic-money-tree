...
const HOLD_PERIODS: Array<{ name: string; minutes: number | null }> = [
  { name: "6h", minutes: 6 * 60 },
  { name: "12h", minutes: 12 * 60 },
...
const EXIT_SCHEDULES: Array<{
...
}> = [
  {
    name: "baseline_1pct",
    targets: [
      { name: "target_1pct", returnPct: 0.01, fraction: 1 },
    ],
  },
  {
    name: "partial_0.5_1_2",
    targets: [
      { name: "target_0.5pct", returnPct: 0.005, fraction: 0.50 },
      { name: "target_1pct", returnPct: 0.01, fraction: 0.25 },
...
const CONFIGURATIONS: Configuration[] = EXIT_SCHEDULES.flatMap(
  (schedule) =>
    HOLD_PERIODS.map((hold) => ({
      name: `${schedule.name}_${hold.name}`,
...
function sweepPortfolioEvents(
  ...
  while (heap.length > 0) {
    ...
    while (current) {
      ...
      // Floating-point arithmetic can leave a fully closed portfolio with a
      // tiny negative residual (for example -7.8e-8). That is numerical
      // round-off, not genuinely negative deployed capital. Clamp only values
      // inside a small tolerance; retain the guard for real accounting errors.
...
function main(): void {
  ...
  const output = {
    ...
    methodology: {
      ...
      exitSchedules: EXIT_SCHEDULES.map((schedule) => schedule.name),
      ...
      optimisation:
        "signals calculated once per market; stop/target exits found by segment-tree first-crossing searches; holding-period deadlines found by binary search; portfolio events merged from sorted per-market streams; chronological mark-to-market uses a market-level heap",
